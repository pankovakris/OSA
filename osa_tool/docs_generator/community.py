...
class CommunityTemplateBuilder:
    ...
    def __init__(self, config_loader: ConfigLoader):
        """
        Initializes the CommunityFilesGenerator with configuration and repository details.
        ...
        self.template_path = os.path.join(
            osa_project_root(), "docs", "templates", "community.toml"
        )
        self.url_path = (
        ...
        self.code_of_conduct_to_save = os.path.join(
            self.repo_path, "CODE_OF_CONDUCT.md"
        )
    ...
    def build_code_of_conduct(self) -> None:
        ...
        try:
            ...
            logger.info(
                f"CODE_OF_CONDUCT.md successfully generated in folder {self.repo_path}"
            )
        except Exception as e:
            logger.error(
                "Error while generating CODE_OF_CONDUCT.md: %s", repr(e), exc_info=True
            )
    ...
    def build_pull_request(self) -> None:
        ...
        try:
            ...
            content = self._template["pull_request"].format(
                contributing_url=contributing_url
            )
            save_sections(content, self.pr_to_save)
            logger.info(
                f"PULL_REQUEST_TEMPLATE.md successfully generated in folder {self.repo_path}"
            )
        except Exception as e:
            logger.error(
    ...
    def build_documentation_issue(self) -> None:
        ...
        try:
            if self.sourcerank.docs_presence():
                ...
                logger.info(
                    f"DOCUMENTATION_ISSUE.md successfully generated in folder {self.repo_path}"
                )
        except Exception as e:
            logger.error(
    ...
    def build_feature_issue(self) -> None:
        ...
        try:
            content = self._template["feature_issue"].format(
                project_name=self.metadata.name
            )
            save_sections(content, self.feature_issue_to_save)
            logger.info(
                f"FEATURE_ISSUE.md successfully generated in folder {self.repo_path}"
            )
        except Exception as e:
            logger.error(
                "Error while generating FEATURE_ISSUE.md: %s", repr(e), exc_info=True
            )

    def build_bug_issue(self) -> None:
        """Generates and saves the BUG_ISSUE.md file."""
        try:
            content = self._template["bug_issue"].format(
                project_name=self.metadata.name
            )
            save_sections(content, self.bug_issue_to_save)
            logger.info(
                f"BUG_ISSUE.md successfully generated in folder {self.repo_path}"
            )
        except Exception as e:
            logger.error(
                "Error while generating BUG_ISSUE.md: %s", repr(e), exc_info=True
            )
